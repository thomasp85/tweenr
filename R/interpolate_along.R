interpolate_numeric_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  numeric_along_interpolator(as.numeric(data), as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes), as.character(ease))
}

interpolate_logical_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  res <- interpolate_numeric_along(data, group, frame, history, keep_last, nframes, ease)
...
interpolate_colour_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  ...
  col <- colour_along_interpolator(data, as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes), as.character(ease))
  data.frame(
...
interpolate_constant_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  constant_along_interpolator(as.character(data), as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes), as.character(ease))
}
...
interpolate_list_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  new_data <- list_along_interpolator(as.list(data), as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes), as.character(ease))
...
interpolate_numlist_along <- function(data, group, frame, nframes, ease, history, keep_last) {
  new_data <- numlist_along_interpolator(lapply(data, as.numeric), as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes), as.character(ease))
  attributes(new_data$data) <- attributes(data)
  new_data
}
get_phase_along <- function(group, frame, nframes, history, keep_last) {
  phase_along_interpolator(as.integer(group), as.numeric(frame), as.logical(history), as.logical(keep_last), as.integer(nframes))
}
